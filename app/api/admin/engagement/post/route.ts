...
import { setPostEngagementOverride } from "@/lib/db/repository";
...
export async function POST(request: Request) {
  try {
    ...
    await setPostEngagementOverride(parsed);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to update post engagement." },
      { status: 400 }
    );
  }
}
